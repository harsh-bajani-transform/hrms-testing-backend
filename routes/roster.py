...
def get_leave_history():
    ...
    try:
        ...
        if role_name == "agent":
            ...
            # Managers can see their team members' leave history
            logged_id_str = str(logged_in_user_id)
        ...
        elif role_name in ["admin", "super admin", "assistant manager", "project manager"]:
            # No additional filtering needed - can see all
            pass
        ...
        for draft in drafts:
            ...
            elif draft["status"].lower() in ["approved", "rejected"] :
                # For all approved/rejected requests (not just leave)
                request_info = {
                    "draft_id": draft["draft_id"],
                    "user_id": draft["user_id"],
                    "user_name": draft["user_name"],
